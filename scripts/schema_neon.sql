...
-- =========================================================
-- USER (Extensions for Better-Auth)
-- =========================================================

-- Better-auth creates the "user" table. We just need to add our custom columns if they don't exist.
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='user' AND column_name='credits') THEN
        ALTER TABLE public."user" ADD COLUMN credits integer not null default 0 check (credits >= 0);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='user' AND column_name='notoriety') THEN
        ALTER TABLE public."user" ADD COLUMN notoriety integer not null default 0 check (notoriety >= 0);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='user' AND column_name='is_verified_student') THEN
        ALTER TABLE public."user" ADD COLUMN is_verified_student boolean not null default false;
    END IF;
END$$;

-- Note: We drop the 'profiles' table concepts entirely, as better-auth manages users in table 'user'.
...
create table if not exists public.quests (
  ...
  hunter_id text references public."user"(id) on delete set null,
...
-- Drop Profiles Table if it exists (legacy)
